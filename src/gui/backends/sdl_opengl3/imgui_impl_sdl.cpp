...
//  2018-XX-XX: Platform: Added support for multiple windows via the
//  ImGuiPlatformIO interface. 2019-04-23: Inputs: Added support for
...
static void ImGui_ImplSDL2_UpdateMousePosAndButtons() {
  ...
  // mouse pos. (rarely used, mostly when ImGuiConfigFlags_NavEnableSetMousePos
  // is enabled by user)
  ...
  // [2]
  // Set Dear ImGui mouse pos from OS mouse pos + get buttons. (this is the
  // common behavior)
  ...
  if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable) {
    ...
    // coordinatesio.MousePos is (0,0) when the mouse is on the upper-left
...
static void ImGui_ImplSDL2_CreateWindow(ImGuiViewport* viewport) {
  ...
  ImGuiViewport* main_viewport = ImGui::GetMainViewport();
  ImGuiViewportDataSDL2* main_viewport_data =
      (ImGuiViewportDataSDL2*)main_viewport->PlatformUserData;
  ...
  sdl_flags |= (viewport->Flags & ImGuiViewportFlags_TopMost)
                   ...
                   : 0;
#endif
  data->Window = SDL_CreateWindow("No Title Yet", (int)viewport->Pos.x,
...
static void ImGui_ImplSDL2_DestroyWindow(ImGuiViewport* viewport) {
  if (ImGuiViewportDataSDL2* data =
          (ImGuiViewportDataSDL2*)viewport->PlatformUserData) {
...
static int ImGui_ImplSDL2_CreateVkSurface(ImGuiViewport* viewport,
  ...
  ImGuiViewportDataSDL2* data =
      (ImGuiViewportDataSDL2*)viewport->PlatformUserData;
  (void)vk_allocator;
  SDL_bool ret = SDL_Vulkan_CreateSurface(data->Window, (VkInstance)vk_instance,
...
static void ImGui_ImplSDL2_InitPlatformInterface(SDL_Window* window,
  ...
  // SDL2 by default doesn't pass mouse clicks to the application when the click
  // focused a window. This is getting in the way of our interactions and we
