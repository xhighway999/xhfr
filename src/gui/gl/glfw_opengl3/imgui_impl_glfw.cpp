...
//  ImGuiPlatformIO interface. 2019-05-11: Inputs: Don't filter value from
...
static void ImGui_ImplGlfw_CreateWindow(ImGuiViewport *viewport) {
  ...
  data->Window             = glfwCreateWindow((int)viewport->Size.x,
                                  (int)viewport->Size.y,
                                  "No Title Yet",
                                  NULL,
                                  share_window);
...
static LRESULT CALLBACK WndProcNoInputs(HWND hWnd,
                                        UINT msg,
                                        WPARAM wParam,
                                        LPARAM lParam) {
  if(msg == WM_NCHITTEST) {
...
#endif
static void ImGui_ImplWin32_SetImeInputPos(ImGuiViewport *viewport,
                                           ImVec2 pos) {
  COMPOSITIONFORM cf = {
      CFS_FORCE_POSITION,
...
static int ImGui_ImplGlfw_CreateVkSurface(ImGuiViewport *viewport,
  ...
  VkResult err =
      glfwCreateWindowSurface((VkInstance)vk_instance,
                              data->Window,
...
// FIXME-PLATFORM: GLFW doesn't export monitor work area (see
// https://github.com/glfw/glfw/pull/989)
static void ImGui_ImplGlfw_UpdateMonitors() {
  ImGuiPlatformIO &platform_io = ImGui::GetPlatformIO();
...
static void ImGui_ImplGlfw_InitPlatformInterface() {
  ...
  // Note: monitor callback are broken GLFW 3.2 and earlier (see
