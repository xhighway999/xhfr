...
const FT_Bitmap* FreeTypeFont::RenderGlyphAndGetInfo(
    GlyphInfo* out_glyph_info) {
  FT_GlyphSlot slot = Face->glyph;
  ...
  for (int src_i = 0; src_i < src_tmp_array.Size; src_i++) {
    ...
    for (int glyph_i = 0; glyph_i < src_tmp.GlyphsCount; glyph_i++) {
      ...
      if (char_advance_x_org != char_advance_x_mod)
        char_off_x +=
            cfg.PixelSnapH
                ? IM_FLOOR((char_advance_x_mod - char_advance_x_org) * 0.5f)
      ...
      dst_font->AddGlyph((ImWchar)src_glyph.Codepoint, x0, y0, x1, y1, u0, v0,
                         u1, v1, char_advance_x_mod);
    }
...
static void (*GImFreeTypeFreeFunc)(void* ptr,
                                   void* user_data) = ImFreeTypeDefaultFreeFunc;
...
static void* FreeType_Realloc(FT_Memory /*memory*/,
                              long cur_size,
                              long new_size,
                              void* block) {
  ...
  if (new_size > cur_size) {
    void* new_block =
        GImFreeTypeAllocFunc((size_t)new_size, GImFreeTypeAllocatorUserData);
...
void ImGuiFreeType::SetAllocatorFunctions(void* (*alloc_func)(size_t sz,
                                                              void* user_data),
